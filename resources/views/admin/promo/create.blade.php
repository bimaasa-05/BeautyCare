...
<head>
    ...
    <title>Tambah Promo - BeautyCare</title>
            ...
            <div class="page-header-premium">
                <div class="ph-content">
                    <div class="ph-left">
                        ...
                        <div class="ph-text">
                            <h3>Buat Promo</h3>
                            <p>Buat promo menarik untuk meningkatkan penjualan.</p>
                        </div>
                    </div>
                </div>
            </div>
                <div class="bg-white rounded-2xl p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            <h3 class="text-[16px] font-bold text-gray-800">Tambah Promo</h3>
                            <p class="text-[12px] text-gray-400 mt-0.5">Buat promo baru</p>
                        </div>
                    ...
                    <form action="{{ route('admin.promo.store') }}" method="POST">
                        ...
                        <div class="space-y-5">
                            <div>
                                ...
                                <input type="text" name="nm_promo" value="{{ old('nm_promo') }}"
                            ...
                            <div>
                                ...
                                <select name="jenis_promo"
                                    ...
                                    <option value="Cashback" {{ old('jenis_promo') == 'Cashback' ? 'selected' : '' }}>Cashback</option>
                                    <option value="Paket" {{ old('jenis_promo') == 'Paket' ? 'selected' : '' }}>Paket</option>
                                    <option value="Buy 1 Get 1" {{ old('jenis_promo') == 'Buy 1 Get 1' ? 'selected' : '' }}>Buy 1 Get 1</option>
                                    <option value="Lainnya" {{ old('jenis_promo') == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                                </select>
                                @error('jenis_promo')
                                    <p class="text-red-500 text-[11px] mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" name="nilai" value="{{ old('nilai', 0) }}" step="0.01" min="0"
                            ...
                            <div>
                                ...
                                <textarea name="deskripsi" rows="3"
                                    ...
                                    placeholder="Contoh: Diskon 20% untuk semua perawatan wajah, berlaku s/d akhir bulan">{{ old('deskripsi') }}</textarea>
                                @error('deskripsi')
                                    <p class="text-red-500 text-[11px] mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                                            ...
                                            @forelse($layanans as $layanan)
                                                <label class="flex items-center gap-2 text-[12px] text-gray-600 cursor-pointer hover:bg-white px-2 py-1 rounded-lg transition-colors">
                                                    <input type="checkbox" name="id_layanan[]" value="{{ $layanan->id_layanan }}"
                                                        ...
                                                        {{ in_array($layanan->id_layanan, old('id_layanan') ?? []) ? 'checked' : '' }}>
                                                    {{ $layanan->nm_layanan }}
                                                </label>
                                            @empty
                                            ...
                                            @forelse($produks as $produk)
                                                <label class="flex items-center gap-2 text-[12px] text-gray-600 cursor-pointer hover:bg-white px-2 py-1 rounded-lg transition-colors">
                                                    <input type="checkbox" name="id_produk[]" value="{{ $produk->id_produk }}"
                                                        class="accent-pink-500"
                                                        {{ in_array($produk->id_produk, old('id_produk') ?? []) ? 'checked' : '' }}>
                                                    {{ $produk->nm_produk }}
                                                </label>
                                            @empty

            ...
            <div class="page-header-premium">
                <div class="ph-content">
                    <div class="ph-left">
                        ...
                        <div class="ph-text">
                            <h3>Edit Supplier</h3>
                            <p>Ubah data pemasok yang sudah ada.</p>
                        </div>
                    </div>
                </div>
            </div>
                <div class="bg-white rounded-2xl p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            <h3 class="text-[16px] font-bold text-gray-800">Edit Supplier</h3>
                            <p class="text-[12px] text-gray-400 mt-0.5">Ubah data supplier</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                <label class="text-[13px] font-semibold text-gray-700 block mb-1.5">Nama
                                    Supplier</label>
                                <input type="text" name="nm_supplier"
                            ...
                            <div>
                                ...
                                <input type="number" name="no_hp" value="{{ old('no_hp', $supplier->no_hp) }}"
                            ...
                            <div class="md:col-span-2">
                                ...
                                <div id="produkRows" class="space-y-2">
                                    @php
                                        $oldProduk = old('produk') ?? $supplier->produk->map(fn($p) => ['id_produk' => $p->id_produk, 'harga_beli' => $p->pivot->harga_beli])->all();
                                        if (empty($oldProduk)) {
                                            $oldProduk = [['id_produk' => '', 'harga_beli' => 0]];
                                        }
                                    @endphp
                                    @foreach ($oldProduk as $index => $row)
                                        <div class="produk-row grid grid-cols-1 md:grid-cols-[1fr_140px_40px] gap-2 items-start">
                                            <select name="produk[{{ $index }}][id_produk]"
                                                ...
                                                @foreach ($produk as $p)
                                                    <option value="{{ $p->id_produk }}" {{ (string) ($row['id_produk'] ?? '') === (string) $p->id_produk ? 'selected' : '' }}>
                                                        {{ $p->nm_produk }} ({{ $p->satuan }})
                                                    </option>
                                                @endforeach
                                            </select>
                                            <input type="number" name="produk[{{ $index }}][harga_beli]" min="0" value="{{ $row['harga_beli'] ?? 0 }}"
                                                class="w-full bg-gray-50 border border-gray-200 text-[13px] rounded-xl px-4 py-2.5 focus:outline-none focus:border-pink-300 focus:bg-white transition-all"

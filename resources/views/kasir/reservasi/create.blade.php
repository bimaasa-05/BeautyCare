...
<head>
    ...
    <style>
        ...
        .custom-select-option-disabled { opacity: .55; cursor: not-allowed; background: #fafafa; }
        .custom-select-option-disabled:hover { background: #fafafa; }
    </style>
</head>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div class="form-group">
                                ...
                                @php
                                    $optsKaryawan = $karyawan->map(function($k) use ($sedangMelayaniDetail) {
                                        ...
                                        return [
                                            'value' => $k->id,
                                            'disabled' => (bool) ($sedangMelayaniDetail[$k->id] ?? null),
                                            'label' => $k->nama . (($sedangMelayaniDetail[$k->id] ?? null) ? ' — Sedang Melayani' : '')
                                        ];
                                    })->values();
                                @endphp
                                <div x-data="searchableSelect()" x-init='init($el.querySelector("select"), @json($optsKaryawan), @json(old("id_karyawan", "")))' class="relative">
                                    <select name="id_karyawan" id="id_karyawan" class="hidden @error('id_karyawan') border-red-400 @enderror">
                                        ...
                                        @foreach ($optsKaryawan as $opt)
                                            <option value="{{ $opt['value'] }}" {{ old('id_karyawan') == $opt['value'] ? 'selected' : '' }} {{ $opt['disabled'] ? 'disabled' : '' }} data-info="{{ $opt['disabled'] ? 'Sedang melayani' : '' }}">
                                                {{ $opt['label'] }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <div class="relative">
                                        <input type="text" x-model="query" @focus="open = true" @click="open = true" @input="onQueryInput()"
                                            @keydown.escape="open = false" @keydown.down.prevent="moveHighlight(1)" @keydown.up.prevent="moveHighlight(-1)"
                                            @keydown.enter.prevent="selectHighlighted()" @blur="setTimeout(() => open = false, 150)"
                                            class="form-input-custom pr-9 @error('id_karyawan') border-red-400 @enderror"
                                            placeholder="Pilih Karyawan" autocomplete="off">
                                        <i class="fa-solid fa-chevron-down absolute right-4 top-1/2 -translate-y-1/2 text-[10px] text-gray-400 pointer-events-none"></i>
                                    </div>
                                    <div x-show="open" x-transition
                                        class="absolute z-50 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden">
                            ...
                            <div class="form-group">
                                ...
                                <select name="jam" id="jamSlot"
                                    ...
                                    <option value="{{ $jam }}" {{ old('jam', \App\Support\BookingSlot::normalJam(date('H:i'))) == $jam ? 'selected' : '' }}>{{ \App\Support\BookingSlot::formatJamIndo($jam) }}</option>
                                    @endforeach
                                </select>
                                @error('jam')
                                    <p class="text-red-500 text-[11px] mt-1">{{ $message }}</p>
                            ...
                            <div id="layanan-rows">
                                <div class="layanan-row grid grid-cols-1 md:grid-cols-12 gap-3 items-end mb-3 p-3 bg-pink-50/30 rounded-xl">
                                    ...
                                    <div class="md:col-span-3">
                                        ...
                                        <input type="text" name="harga[]" class="form-input-custom harga-input" placeholder="Rp 0" readonly>
                                    </div>
                                    <div class="md:col-span-3">
                                        <label class="text-[11px] font-medium text-gray-500 mb-1 block">Diskon</label>
                                        <input type="text" name="diskon[]" class="form-input-custom diskon-input" placeholder="Rp 0" value="Rp 0" oninput="hitungSubtotal(this)" onblur="formatDiskon(this)">
                                    </div>

...
<head>
    ...
    <style>
        ...
        .struk-line, .struk-row, .struk-total, .struk-footer, .struk-header { display: none; }
        ...
        @media print {
            ...
            .struk-area .no-print { display: none !important; }
            .struk-header { display: block !important; text-align: center; margin-bottom: 10px; }
            .struk-header h2 { font-size: 16px; font-weight: bold; margin: 0; }
            .struk-header p { font-size: 11px; margin: 2px 0; }
            .struk-line { display: block !important; border-top: 1px dashed #000; margin: 6px 0; }
            .struk-row { display: flex !important; justify-content: space-between; padding: 2px 0; font-size: 12px; }
            .struk-total { display: flex !important; justify-content: space-between; font-size: 14px; font-weight: bold; border-top: 1px solid #000; padding-top: 4px; margin-top: 4px; }
            .struk-footer { display: block !important; text-align: center; margin-top: 10px; font-size: 11px; }
        }
    </style>
</head>
            ...
            <div class="flex-1 overflow-y-auto p-8">
                <div class="bg-white rounded-2xl p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] relative overflow-hidden">
                    ...
                    <div class="struk-header">
                        <h2 style="margin:0;font-size:16px;font-weight:bold;">BEAUTYCARE SALON</h2>
                        <p style="margin:2px 0;font-size:11px;">Jl. Contoh No. 123</p>
                        ...
                        <div class="lg:col-span-2">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                <div class="info-box">
                                    ...
                                    <p class="info-value">
                                        @if($transaksi->metode_byr == 'Tunai') 💵 {{ $transaksi->metode_byr }}
                                        @elseif($transaksi->metode_byr == 'Qris') 📱 {{ $transaksi->metode_byr }}
                                        @elseif($transaksi->metode_byr == 'E-Wallet') 📲 {{ $transaksi->metode_byr }} ({{ $transaksi->bank_asal ?? '-' }})
                                        @elseif($transaksi->metode_byr == 'Transfer') 🏦 {{ $transaksi->metode_byr }}
                                        @else 💳 {{ $transaksi->metode_byr }} @endif
                                    </p>
                                </div>
                                <div class="info-box">
                                ...
                                @if($transaksi->metode_byr === 'Qris')
                                <div class="info-box md:col-span-2 bg-purple-50/30 border-purple-100/50">
                                    <p class="info-label"><i class="fa-solid fa-qrcode mr-1 text-purple-400"></i> Pembayaran QRIS</p>
                                    <p class="info-value text-purple-600">
                                        <i class="fa-regular fa-circle-check text-green-500 mr-1"></i>
                                        Dibayar via QRIS — Pelanggan bayar mandiri
                                    </p>
                                </div>
                                @else
                                <div class="info-box">
                            ...
                            <div class="mt-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div class="info-box bg-purple-50/30 border-purple-100/50">
                                        <p class="info-label"><i class="fa-regular fa-user mr-1 text-purple-400"></i> Atas Nama</p>
                                        <p class="info-value">{{ $transaksi->atas_nama }}</p>
                                    </div>
                                    @if($transaksi->no_referensi)
                                    <div class="info-box bg-purple-50/30 border-purple-100/50">
                                        <p class="info-label"><i class="fa-regular fa-hashtag mr-1 text-purple-400"></i> No. Referensi</p>
                    ...
                    <div class="struk-row">
                        <span>Metode:</span>
                        <span>{{ $transaksi->metode_byr }}</span>
                    </div>
                    <div class="struk-row">
                        <span>Dibayar:</span>
                    ...
                    <div class="struk-footer">
                        Terima kasih telah berbelanja di BeautyCare!
                    </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 mt-6 pt-4 border-t border-pink-100/50 no-print">
                        ...
                        <form action="{{ route('kasir.transaksi.destroy', $transaksi->id_transaksi) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus transaksi {{ $transaksi->no_invoice }}?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit"

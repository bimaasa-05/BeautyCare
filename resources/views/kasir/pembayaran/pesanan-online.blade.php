        ...
        <main class="main-content">
            ...
            <div class="flex-1 overflow-y-auto p-8">
                <div
                    ...
                    @if (session('error'))
                        <div
                            class="mb-4 p-4 bg-red-50 border border-red-200 text-red-700 text-[13px] rounded-xl flex items-center gap-2">
                    ...
                    <div class="mb-6 flex items-center justify-between flex-wrap gap-3">
                        <div>
                            ...
                            <p class="text-[12px] text-gray-400 mt-0.5">Verifikasi pembayaran pesanan produk dari
                                pelanggan</p>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-5">
                        <div class="stat-card-enhanced card-gradient-blue">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Menunggu
                                        Verifikasi</p>
                                    <p class="text-[24px] font-bold text-blue-600 mt-1">
                                        {{ $pesanan->where('status', 'Sedang Diproses')->count() }}</p>
                                </div>
                        ...
                        <div class="stat-card-enhanced card-gradient-pink">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Total
                                        Pesanan Aktif</p>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse table-card-mobile">
                            <thead>
                                <tr
                                    ...
                                    <tr class="hover:bg-pink-50/40 transition-all">
                                        ...
                                        <td class="py-3 px-4" data-label="No. Invoice">
                                            <span
                                                class="inline-flex items-center gap-1.5 bg-pink-50 text-pink-500 text-[11.5px] font-semibold rounded-lg px-2.5 py-1">
                                        ...
                                        <td class="py-3 px-4 text-[12px]" data-label="Tanggal">
                                            ...
                                            @if ($p->id_booking && $p->booking)
                                                @php
                                                    $bkJamMulai = \Carbon\Carbon::parse($p->booking->jam)->format(
                                                        'H:i',
                                                    );
                                                    $bkDurasi = \App\Support\BookingSlot::durasiBooking($p->booking);
                                                    $bkJamSelesai = \Carbon\Carbon::parse(
                                                        $p->booking->tanggal . ' ' . substr($p->booking->jam, 0, 5),
                                                    )
                                                        ->addMinutes($bkDurasi)
                                                        ->format('H:i');
                                                @endphp
                                                <div
                                                    class="text-[11px] text-violet-500 font-semibold flex items-center gap-1 mt-1">
                                                    <i class="fa-regular fa-clock"></i>
                                                    Treatment: <span class="font-mono">{{ $bkJamMulai }} -
                                                        {{ $bkJamSelesai }}</span>
                                                </div>
                                            @endif
                                        </td>
                                        <td class="py-3 px-4" data-label="Metode">
                                            @if (($p->pembayaran->metode ?? null) === 'Saldo')
                                                <div
                                                    class="inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-600 text-[11.5px] font-semibold rounded-lg px-2.5 py-1">
                                                    <i class="fa-solid fa-wallet text-[10px]"></i> Saldo Akun
                                                </div>
                                                <div class="text-[11px] text-gray-400 flex items-center gap-1.5 mt-1">
                                                    Pakai saldo: Rp
                                                    {{ number_format($p->saldo_terpakai, 0, ',', '.') }}
                                                </div>
                                            @else
                                        ...
                                        <td class="py-3 px-4" data-label="">
                                            <div class="flex items-center justify-center gap-2">
                                                <a href="{{ route('kasir.pembayaran.show', $p->id_transaksi) }}"
                                                    class="btn-aksi btn-detail">
                                                    <i class="fa-solid fa-eye text-[11px]"></i> Detail
                                                </a>
                                                @php $isBooking = (bool) $p->id_booking; @endphp
                                                <form
                                                    action="{{ route('kasir.pembayaran.verifikasi', $p->id_transaksi) }}"
                                                    method="POST">
                                                    @csrf
                                                    <input type="hidden" name="aksi" value="konfirmasi">
                                                    @if ($demoMode)
                                                        <button type="submit" class="btn-aksi btn-konfirmasi"
                                                            data-confirm-title="Konfirmasi Lunas"
                                                            data-confirm-body="Konfirmasi pembayaran pesanan {{ $p->no_invoice }} sudah diterima dan lunas? Stok akan dikurangi."
                                                            data-confirm-icon="fa-circle-check"
                                                            data-confirm-type="success"
                                                            data-confirm-yes="Ya, Konfirmasi">
                                                            <i class="fa-solid fa-circle-check text-[11px]"></i>
                                                            Konfirmasi
                                                        </button>
                                                    @else
                                                        <button type="submit" class="btn-aksi btn-konfirmasi"
                                                            data-confirm-title="{{ $isBooking ? 'Konfirmasi Pembayaran' : 'Konfirmasi Lunas' }}"
                                                            data-confirm-body="{{ $isBooking ? 'Konfirmasi pembayaran booking ' . $p->no_invoice . ' sudah diterima?' : 'Konfirmasi pesanan ' . $p->no_invoice . ' sudah lunas? Stok akan dikurangi.' }}"
                                                            data-confirm-icon="fa-circle-check"
                                                            data-confirm-type="success"
                                                            data-confirm-yes="Ya, Konfirmasi">
                                                            <i class="fa-solid fa-check text-[11px]"></i>
                                                            {{ $isBooking ? 'Konfirmasi Pembayaran' : 'Konfirmasi Lunas' }}
                                                        </button>
                                                    @endif
                                                </form>
                                                <form
                                                    ...
                                                    <button type="submit" class="btn-aksi btn-tolak"
                                                        ...
                                                        data-confirm-body="Tolak pembayaran {{ $isBooking ? 'booking ' : 'pesanan ' }}{{ $p->no_invoice }}? Saldo akun (jika dipakai) akan dikembalikan."
                                ...
                                <tr>
                                    <td colspan="8" class="py-16 text-center">
                                            ...
                                            <p class="font-semibold text-gray-700 text-[14px]">Tidak Ada Pesanan Online
                                            </p>
                                            <p class="text-[12px] text-gray-400 mt-1">Pesanan dari pelanggan akan
                                                muncul di sini</p>
                                        </td>
                                    </tr>
                                @endforelse

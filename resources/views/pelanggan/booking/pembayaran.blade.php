...
<head>
    ...
    <title>Pembayaran Booking - BeautyCare</title>
    ...
    .booking-info .bi-item {
        display: flex;
        align-items: center;
        gap: 10px;
    }

    .booking-info .bi-item .bi-icon {
        width: 32px;
        height: 32px;
        border-radius: 10px;
        background: #FFF5F8;
...
<body>
    <div class="dashboard-layout">
        ...
        <main class="main-content">
            ...
            <div class="dashboard-content">
                    ...
                    <div class="ph-content">
                        <div class="ph-left">
                            ...
                            <div class="ph-text">
                                ...
                                <p>Periksa rincian booking lalu pilih metode pembayaran</p>
                            </div>
                        </div>
                        <a href="{{ route('pelanggan.booking.detail', $booking->id_booking) }}" class="btn-back">
                ...
                <form action="{{ route('pelanggan.booking.bayar', $booking->id_booking) }}" method="POST" id="bookingBayarForm">
                    @csrf
                    <input type="hidden" name="tipe" id="inpTipe" value="dp">
                        ...
                        <div class="checkout-card">
                            <div class="cc-header">
                                ...
                                <div>
                                    ...
                                    <div class="cc-subtitle">#BK{{ str_pad($booking->id_booking, 3, '0', STR_PAD_LEFT) }} &bull; {{ count($booking->detail) }} layanan</div>
                                </div>
                            </div>
                            <div class="cc-body">
                                <div class="booking-info">
                                    <div class="bi-item">
                                        <div class="bi-icon"><i class="fa-regular fa-calendar"></i></div>
                                        <div>
                                            <div class="bi-label">Tanggal</div>
                                    ...
                                    <div class="bi-item" style="grid-column: 1 / -1;">
                                        <div class="bi-icon"><i class="fa-solid fa-user"></i></div>
                                        <div>
                                            <div class="bi-label">Terapis</div>
                                            <div class="bi-value">{{ $booking->karyawan ? $booking->karyawan->nama : 'Terapis #' . $booking->id_karyawan }}</div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="co-item">
                                    <div class="coi-info">
                                        <div class="coi-nama">{{ $d->layanan->nm_layanan ?? 'Layanan' }}</div>
                                        <div class="coi-meta">
                                            @if(($d->diskon ?? 0) > 0)
                                            Rp {{ number_format($d->harga, 0, ',', '.') }} - Diskon Rp {{ number_format($d->diskon, 0, ',', '.') }}
                                            @else
                                            Rp {{ number_format($d->harga, 0, ',', '.') }}
                                            @endif
                                        </div>
                                    </div>
                                    <div class="coi-harga">Rp {{ number_format($d->subtotal ?? 0, 0, ',', '.') }}</div>
                                </div>
                                @endforeach
                                ...
                                <div class="co-row">
                                    <div>Total Harga</div>
                                    <div class="co-val">Rp {{ number_format($booking->detail->sum('harga'), 0, ',', '.') }}</div>
                                </div>
                                <div class="co-row">
                                    <div>Diskon</div>
                                    <div class="co-val">- Rp {{ number_format($booking->detail->sum('diskon'), 0, ',', '.') }}</div>
                                </div>
                                <div class="co-row">
                                    <div>Subtotal</div>
                                    <div class="co-val">Rp {{ number_format($total, 0, ',', '.') }}</div>
                                </div>

                                <div class="co-row co-row-total">
                                    <div>Total Bayar Sekarang</div>
                                    <div class="co-val" id="ringkasTotal">Rp {{ number_format($dpAmount, 0, ',', '.') }}</div>
                                </div>

                                <div class="co-row" id="rowSisaDp">
                                    <div>Sisa dibayar di salon</div>
                                    <div class="co-val" id="ringkasSisaDp">Rp {{ number_format($sisa, 0, ',', '.') }}</div>
                                </div>
                        ...
                        <div class="checkout-card">
                            <div class="cc-header">
                                ...
                                <div>
                                    ...
                                    <div class="cc-subtitle">Pilih skema & metode pembayaran</div>
                                </div>
                            </div>
                            <div class="cc-body">
                                <div class="alert-dp-note" id="alertDpNote">
                                    <i class="fa-solid fa-circle-info"></i>
                                    <div>
                                        Anda memilih <b>DP 50%</b>. Sisa <b>Rp <span id="alertDpSisa">{{ number_format($sisa, 0, ',', '.') }}</span></b>
                                        dibayar di salon saat treatment selesai.
                                    </div>
                                </div>

                                <div class="tipe-group">
                                    <div class="tg-title"><i class="fa-solid fa-coins"></i> Skema Pembayaran</div>
                                    <label class="pay-option pay-option-tipe selected" data-tipe="dp">
                                        <input type="radio" name="pay_tipe" value="dp" checked>
                                        <div class="po-icon"><i class="fa-solid fa-money-bill-wave"></i></div>
                                        <div>
                                            <div class="po-label">Bayar DP 50%</div>
                                            <div class="po-desc">Rp {{ number_format($dpAmount, 0, ',', '.') }} sekarang, sisa di salon</div>
                                        </div>
                                    </label>
                                    <label class="pay-option pay-option-tipe" data-tipe="full">
                                        <input type="radio" name="pay_tipe" value="full">
                                        <div class="po-icon"><i class="fa-solid fa-circle-check"></i></div>
                                        <div>
                                            <div class="po-label">Bayar Lunas</div>
                                            <div class="po-desc">Rp {{ number_format($total, 0, ',', '.') }} dibayar penuh sekarang</div>
                                        </div>
                                    </label>
                                </div>
                                ...
                                <button type="submit" class="btn-buat-pesanan" id="btnBayarBooking" disabled>
    ...
    <script>
    var totalPenuh = parseInt('{{ $total }}');
    var dpAmount = parseInt('{{ $dpAmount }}');
    var sisaDp = parseInt('{{ $sisa }}');
    var saldoTersedia = parseInt('{{ $saldo ?? 0 }}');

    var tipe = 'dp';
    var totalBayar = dpAmount;

    var saldoTerpakai = 0;
    var metodeKedua = null;
    ...
    function setTotalBayar() {
        if (tipe === 'dp') {
            totalBayar = dpAmount;
            document.getElementById('ringkasTotal').textContent = 'Rp ' + fmtAngka(dpAmount);
            document.getElementById('rowSisaDp').style.display = 'flex';
            document.getElementById('ringkasSisaDp').textContent = 'Rp ' + fmtAngka(sisaDp);
            document.getElementById('alertDpNote').style.display = 'flex';
            document.getElementById('alertDpSisa').textContent = fmtAngka(sisaDp);
        } else {
            totalBayar = totalPenuh;
            document.getElementById('ringkasTotal').textContent = 'Rp ' + fmtAngka(totalPenuh);
            document.getElementById('rowSisaDp').style.display = 'none';
            document.getElementById('alertDpNote').style.display = 'none';
        }
        hitungRingkasan();
    }
    ...
    function updateButtonState() {
        var btn = document.getElementById('btnBayarBooking');
        var saldoChecked = document.querySelector('.pay-option-saldo input').checked;
        var pakaiSaldo = saldoChecked ? Math.max(0, Math.min(saldoTersedia, totalBayar)) : 0;
        var sisaBayar = totalBayar - pakaiSaldo;

        var alertEl = document.getElementById('alertSaldoKurang');
        var nonSaldoOpts = document.querySelectorAll('.pay-option:not(.pay-option-saldo):not(.pay-option-tipe)');

        if (saldoChecked && saldoTersedia < totalBayar) {
            document.getElementById('alertSaldoNom').textContent = 'Rp ' + fmtAngka(saldoTersedia);
            document.getElementById('alertSaldoTotal').textContent = 'Rp ' + fmtAngka(totalBayar);
    ...
    document.querySelectorAll('.pay-option-tipe').forEach(function(opt) {
        opt.addEventListener('click', function(e) {
            e.preventDefault();
            if (opt.classList.contains('disabled')) return;
            clearSelection('.pay-option-tipe');
            opt.classList.add('selected');
            opt.querySelector('input').checked = true;
            tipe = opt.getAttribute('data-tipe');
            document.getElementById('inpTipe').value = tipe;
            setTotalBayar();
        });
    });
    ...
    document.querySelectorAll('.pay-option:not(.pay-option-saldo):not(.pay-option-tipe)').forEach(function(opt) {
        opt.addEventListener('click', function(e) {
            e.preventDefault();
            if (opt.classList.contains('disabled')) return;
            var input = opt.querySelector('input');
            ...
            if (wasChecked) {
                clearSelection('.pay-option:not(.pay-option-saldo):not(.pay-option-tipe)');
                metodeKedua = null;
                providerKedua = null;
                bankIdKedua = null;
                document.getElementById('inpMetode').value = '';
    ...
    document.getElementById('bookingBayarForm').addEventListener('submit', function(e) {

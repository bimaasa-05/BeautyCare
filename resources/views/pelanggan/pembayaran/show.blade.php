...
<body>
            ...
            <div class="dashboard-content">
                ...
                <div class="pay-main">
                    <div class="status-card">
                        ...
                        @if ($transaksi->status === 'Menunggu Pembayaran')
                            <div class="countdown-box">
                                ...
                                <div class="cd-time" id="countdown"
                                    data-expires="{{ $transaksi->pembayaran->expires_at->toIso8601String() }}">--:--:--
                                </div>
                            </div>
                        @else
                            <div class="waiting-box">
                                <div class="wb-title"><i class="fa-solid fa-circle-notch"></i> Menunggu Verifikasi Kasir
                                </div>
                                <div class="wb-desc">
                                    @if ($transaksi->pembayaran->metode === 'Saldo')
                                        Pembayaran Anda sudah dilunasi dengan saldo akun. Kasir akan memverifikasi
                                        pesanan Anda.
                                    @else
                                        Konfirmasi pembayaran Anda sudah terkirim. Kasir akan memverifikasi pesanan
                                        Anda.
                                    @endif
                                </div>
                            </div>
                    ...
                    @if (in_array($transaksi->status, ['Menunggu Pembayaran', 'Sedang Diproses']) &&
                            $transaksi->pembayaran->metode !== 'Saldo')
                        <div class="pay-card">
                            <div class="pc-header">
                                <div class="pc-icon"><i class="fa-solid fa-paperclip"></i></div>
                                <div>
                                    <div class="pc-title">Upload Bukti Pembayaran</div>
                                    <div class="pc-subtitle">Screenshot bukti transfer / pembayaran Anda (wajib, maks 2
                                        MB)</div>
                                </div>
                            </div>
                            <div class="pm-box">
                                @if ($transaksi->bukti_bayar)
                                    <div style="text-align:center;margin-bottom:14px;">
                                        <img src="{{ Storage::url($transaksi->bukti_bayar) }}" alt="Bukti bayar"
                                            style="max-width:220px;border-radius:14px;border:1.5px solid var(--border);">
                                        <div style="font-size:11px;color:var(--gray);margin-top:6px;">Bukti yang sudah
                                            diunggah. Ganti dengan yang baru jika diperlukan.</div>
                                    </div>
                                @endif
                                <form action="{{ route('pelanggan.pembayaran.bukti', $transaksi->id_transaksi) }}"
                                    method="POST" enctype="multipart/form-data" id="formBukti">
                                    @csrf
                                    <input type="file" name="bukti_bayar" id="inpBukti"
                                        accept="image/jpeg,image/png,image/jpg" class="bukti-input"
                                        onchange="pilihBukti(this)">
                                    <div class="bukti-pick" id="buktiPick"
                                        onclick="document.getElementById('inpBukti').click()">
                                        <i class="fa-solid fa-image"></i>
                                        <span
                                            id="buktiLabel">{{ $transaksi->bukti_bayar ? 'Ganti Bukti Pembayaran' : 'Upload Bukti Pembayaran' }}</span>
                    ...
                    @if ($transaksi->status === 'Menunggu Pembayaran')
                        <div class="pay-actions">
                            <form action="{{ route('pelanggan.pembayaran.sudah-bayar', $transaksi->id_transaksi) }}"
                                method="POST">
                                @csrf
                                <button type="submit" class="btn-pay btn-pay-confirm" style="width:100%;"
                                    {{ $transaksi->bukti_bayar ? '' : 'disabled' }}
                                    {{ $transaksi->bukti_bayar ? '' : 'title="Upload bukti pembayaran terlebih dahulu agar tombol ini aktif."' }}
                                    data-confirm-title="Saya Sudah Bayar"
                                    data-confirm-body="Pastikan Anda sudah melakukan pembayaran sesuai nominal. Lanjutkan?"
                                    data-confirm-icon="fa-money-check-dollar" data-confirm-type="success"
                                    data-confirm-yes="Ya, Sudah Bayar">
                                    <i class="fa-solid fa-check-circle"></i> Saya Sudah Bayar
                                </button>
                            </form>
                            <form action="{{ route('pelanggan.pembayaran.batal', $transaksi->id_transaksi) }}"
                                method="POST">
                                @csrf
                                <button type="submit" class="btn-pay btn-pay-batal" style="width:100%;"
                                    data-confirm-title="Batalkan Pesanan"
                                    data-confirm-body="Apakah Anda yakin ingin membatalkan pesanan ini? Tindakan ini tidak dapat dibatalkan."
                                    data-confirm-icon="fa-circle-xmark" data-confirm-type="danger"
    ...
    <div class="expired-modal" id="expiredModal">
        <div class="em-card">
            ...
            <div class="em-desc">Pembayaran pesanan <b>{{ $transaksi->no_invoice }}</b> melewati batas waktu. Anda
                bisa memperpanjang waktu atau membatalkan pesanan.</div>
            <div class="em-actions">
                <form action="{{ route('pelanggan.pembayaran.perpanjang', $transaksi->id_transaksi) }}"
                ...
                <form action="{{ route('pelanggan.pembayaran.batal', $transaksi->id_transaksi) }}" method="POST"
                    ...
                    @csrf
                    <button type="submit" class="em-btn em-btn-batal" data-confirm-title="Batalkan Pesanan"
                        data-confirm-body="Apakah Anda yakin ingin membatalkan pesanan ini? Tindakan ini tidak dapat dibatalkan."
                        data-confirm-icon="fa-circle-xmark" data-confirm-type="danger"
                        data-confirm-yes="Ya, Batalkan">
    ...
    <script>
        var expiresAt = document.getElementById('countdown') ? new Date(document.getElementById('countdown').getAttribute(
            'data-expires')) : null;
        ...
        function salinVA() {
            ...
            navigator.clipboard.writeText(va).then(function() {
                ...
                setTimeout(function() {
                    ...
                    btn.innerHTML = '<i class="fa-regular fa-copy"></i> ' + (btn.getAttribute(
                        'data-label') || 'Salin');
                }, 2000);
            });
        }
